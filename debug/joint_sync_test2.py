...
m_home_sim = sim.getObjectMatrix(ee_handle, sim.handle_world)
T_home_fk = compute_fk_dh(home_angles)
home_sim_pos = np.array([m_home_sim[3], m_home_sim[7], m_home_sim[11]])

print(f"Joint angles: {home_angles}")
print(f"EE pos (sim):  {home_sim_pos}")
print(f"EE pos (FK):   {T_home_fk[:3,3]}")
print(f"Difference:    {home_sim_pos - T_home_fk[:3,3]}")
...
for j, angle in zip(joint_handles, test_angles_1):
    sim.setJointPosition(j, angle)
time.sleep(0.5)

# Read what was actually set
read_angles_1 = np.array([sim.getJointPosition(j) for j in joint_handles])
m_pose1_sim = sim.getObjectMatrix(ee_handle, sim.handle_world)
T_pose1_fk = compute_fk_dh(read_angles_1)
pose1_sim_pos = np.array([m_pose1_sim[3], m_pose1_sim[7], m_pose1_sim[11]])
...
print(f"  Sim:         {pose1_sim_pos}")
print(f"  Error (Sim - FK): {pose1_sim_pos - T_pose1_fk[:3,3]}")
...
for j, angle in zip(joint_handles, test_angles_2):
    sim.setJointPosition(j, angle)
time.sleep(0.5)

read_angles_2 = np.array([sim.getJointPosition(j) for j in joint_handles])
m_pose2_sim = sim.getObjectMatrix(ee_handle, sim.handle_world)
T_pose2_fk = compute_fk_dh(read_angles_2)
pose2_sim_pos = np.array([m_pose2_sim[3], m_pose2_sim[7], m_pose2_sim[11]])
...
print(f"  Sim:         {pose2_sim_pos}")
print(f"  Error (Sim - FK): {pose2_sim_pos - T_pose2_fk[:3,3]}")
...
pose1_error = np.linalg.norm(pose1_sim_pos - T_pose1_fk[:3,3])
pose2_error = np.linalg.norm(pose2_sim_pos - T_pose2_fk[:3,3])
...
home_error = np.linalg.norm(home_sim_pos - T_home_fk[:3,3])
print(f"   Home: Sim-FK = {home_error*1000:.1f} mm")
